...
## 4. What gets traced — two layers, showing up as two separate traces per request
...
1. **`ask_janmitra_graph`** — this app's own hand-built spans (`backend/services/observability/
   ...
   ₹ cost, model name, and token counts — it's what the "Cost by model" panel (§6) reads.
...
## 5. Keeping the two tracing backends genuinely independent
...
## 6. Real ₹ cost tracking — the "Cost by model" panel
...
## 7. Sessions, retention, and review annotations
...
## 8. Admin alerts — "High AI latency" / "High AI error rate"
...
## 10. Likely interview questions about this part of the project
